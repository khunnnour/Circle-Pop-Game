...
public class BoardManager : MonoBehaviour
{
	...
	private bool _inited; // if board is currently intractable
	...
	public void Init(Vector2Int bS, Vector2 sS, int nC)
	{
		...
		// scale is some fraction of the smaller of the cell dimensions (in case rectangular)
		Vector3 circleScale = Vector3.one * Mathf.Min(_cellDimensions.x, _cellDimensions.y) * 0.95f;
		...
		{
			GameObject newCell = Instantiate(circlePref, transform);                    // instantiate the prefab
			newCell.transform.localPosition = MapCoordToWorldPos(IndexToMapCoord(i));   // put in correct position
			newCell.transform.localScale = circleScale;                                 // set to correct scale
			int colIn = Random.Range(0, _numCols);                                      // give random intial color
			_cells[i] = newCell.GetComponent<Circle>();                                 // get reference to circle script
			_cells[i].Init(i, colIn, colors[colIn]);                                    // initialize the circle
		}
	...
	{
		if (!_inited) return; // check if _cells is inited
